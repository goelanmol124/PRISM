...
import whisper
import torch
from llm_core import call_llm_with_structure, AnalysisResult, HeadingResult, CaptionAnalysis, CaptionGroup
...
def _crop_to_vertical(clip, start_time=None, face_x_offset=None):
    ...
    try:
        ...
        for cut in cuts:
            ...
            if end > start:
                ...
                clip = _crop_to_vertical(clip, start, face_x_offset)
...
workflow.add_node("analyze_visually", analyze_video_visually)  # NEW: Vision LLM analysis
...
# Updated flow: extract -> transcribe -> visual analysis (parallel with transcript analysis)
workflow.add_edge("extract_audio", "transcribe")
workflow.add_edge("transcribe", "analyze_visually")  # Visual analysis after transcription
workflow.add_edge("analyze_visually", "analyze")     # Then transcript analysis (can use vision context)
